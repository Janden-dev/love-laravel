        ...
        <section class="page active">
            ...
            <div class="card">
                <div class="cal-header">
                    <a href="{{ route('diary.index', ['year' => $current->copy()->subMonth()->year, 'month' => $current->copy()->subMonth()->month, 'date' => $selectedDate]) }}">‹</a>
                    <span>{{ $year }} 年 {{ $month }} 月</span>
                    <a href="{{ route('diary.index', ['year' => $current->copy()->addMonth()->year, 'month' => $current->copy()->addMonth()->month, 'date' => $selectedDate]) }}">›</a>
                </div>
                ...
                <div class="cal-grid">
                    ...
                    @for($d = 1; $d <= $daysInMonth; $d++)
                        ...
                        @endphp
                        <a href="{{ route('diary.index', ['year' => $year, 'month' => $month, 'date' => $ds]) }}"
            ...
            <div class="card diary-view">
                ...
                <div class="d-date">📅 {{ $selectedCarbon->format('Y.m.d') }} {{ $isToday ? '（今天）' : '' }}</div>
